...
const targetIdx = runE2E.indexOf("✅ ALL E2E TESTS PASSED SUCCESSFULLY");
if (targetIdx === -1) {
    console.error("Could not find the target string!");
    process.exit(1);
}

// targetIdx points to the ✅ character
// We actually want to replace the whole console.log line to be safe.
// So let's just insert our code before the console.log statement.
const insertIdx = runE2E.lastIndexOf("console.log(", targetIdx);

const before = runE2E.substring(0, insertIdx);
const after = runE2E.substring(insertIdx);
...
const newTests = `
            ...
            await page.evaluate(() => {
                const gs = window.__PHASER_GAME__.scene.scenes.find(s => s.scene.key === 'GameScene');
                gs.player.hp = 0;
                gs.player.takeDamage(0, 0, new Phaser.Math.Vector2(0,0));
...
fs.writeFileSync('tests/e2e/run-e2e.cjs', before + newTests + after);
