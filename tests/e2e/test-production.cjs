...
const baseURL = 'https://chaohuang-TW.github.io/number-snake-arena/';
...
(async () => {
    ...
    const runTests = async () => {
        try {
            ...
            await page.evaluate(() => { API.setPlayerValue(10); API.setPlayerHP(3); API.spawnEnemy(24, API.getPlayerPos().x, API.getPlayerPos().y); API.forceCollisionWithEnemy(0); });
            let hpC2 = await page.evaluate(() => API.getPlayerHP());
            assert(hpC2 === 1, `HP should be 1, got ${hpC2}`);
            await cleanEnemies();
            await page.waitForTimeout(1500);

            await page.evaluate(() => { API.setPlayerValue(10); API.setPlayerHP(3); API.spawnEnemy(26, API.getPlayerPos().x, API.getPlayerPos().y); API.forceCollisionWithEnemy(0); });
            ...
            for (const v of viewports) {
                ...
                assert(speedBoost > speedNormal * 1.5, `Speed should increase on ${v.width}, got boost ${speedBoost} normal ${speedNormal}`);
